...
colnames(FC) <- paste('middle.temporal',colnames(FC),sep='.')
FC <- rownameToFirstColumn(FC,'GeneSymbol')

PVAL <- DIFF.EXP$DEXP$adj.P.Val
colnames(PVAL) <- paste('middle.temporal',colnames(PVAL),sep='.')
PVAL <- rownameToFirstColumn(PVAL,'GeneSymbol')
```
...
FC <- join(FC, rownameToFirstColumn(tmp,'GeneSymbol'), by = 'GeneSymbol', match = 'all')

tmp <- DIFF.EXP$DEXP$adj.P.Val
colnames(tmp) <- paste('superior.temporal',colnames(tmp),sep='.')
PVAL <- join(PVAL, rownameToFirstColumn(tmp,'GeneSymbol'), by = 'GeneSymbol', match = 'all')
```
