...
# knitFile2Synapse(file = "./DiffExpAnal_ROSMAP_RNASeq.Rmd", owner = '', name = "Differential Expression Analysis ROSMAP RNASeq",overwrite=F)
...
METADATA.CLINICAL <- read.table(METADATA.CLINICAL_OBJ@filePath,sep=',',header=T, row.names=1)
...
EXPR <- merge(COUNT1,COUNT2,by = 'gene_id',all=T)
rownames(EXPR) <- EXPR$gene_id
EXPR <- log2(dplyr::select(EXPR,-gene_id,-tracking_id.x,-tracking_id.y))

# Match covariates to expression data
METADATA.TECH <- dplyr::mutate(METADATA.TECH, user.id = paste(Sampleid,Batch,sep='_'))
rownames(METADATA.TECH) <- METADATA.TECH$user.id

ind <- match(METADATA.TECH$user.id,colnames(EXPR))
EXPR <- EXPR[,ind]

ind <- match(colnames(EXPR),METADATA.TECH$user.id)
METADATA.TECH <- METADATA.TECH[ind,]

ind <- match(METADATA.TECH$Projid,rownames(METADATA.CLINICAL))
METADATA.CLINICAL <- METADATA.CLINICAL[ind,]
...
FactorCovariates <- c( "age_first_ad_dx2"       "age_death2"             "age_at_visit_max2"      "pmi"                    "msex"                   "educ"                  
 [7] "apoe_genotype"          "race"                   "spanish"                "braaksc"                "cts_mmse30_first_ad_dx" "cts_mmse30_lv"         
[13] "ceradsc"                "cogdx" )
ContCovariates <- c('RINcontinuous',)
  
# Find inter relation between factor covariates
COVARIATES = cbind(METADATA.CLINICAL,METADATA.TECH[,c('RINcontinuous'),drop=F])
