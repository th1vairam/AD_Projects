...
# knitFile2Synapse(file = "./DiffExpAnal_MSSM-RNASeq.Rmd", owner = 'syn4210618', name = "Differential Expression Analysis MSSM RNASeq",overwrite=F)
...
4. Data were obtained from AMP-AD portal of MSSM, and following brain regions were analysed for differential expression between case and control:
    * Frontal Pole, 
    * Superior Temporal Gyrus, [significantly correlated]
...
FactorCovariates <- c('TISSUE','Donor','RACE','CDR','CERJ','SEX','NP.1','bbscore')
...
COVARIATES <- dplyr::mutate(COVARIATES, TISSUE.CDR = paste(TISSUE,CDR,sep='.'))
COVARIATES$TISSUE.CDR <- factor(COVARIATES$TISSUE.CDR)
```
...
Reperform normalisation with custom design matrix (manually with "TISSUE.CDR","SEX","RACE","PMI" as fixed effects and Donor as random effects)

```{r reperform.normalisation, cache=TRUE}
# Post adjusted design matrix
DM1 = getDesignMatrix(COVARIATES[,c("TISSUE.CDR","SEX","RACE","PMI"),drop=F],Intercept = F)
...
colnames(CONT) <- gsub('TISSUE.CDR','',colnames(CONT))
