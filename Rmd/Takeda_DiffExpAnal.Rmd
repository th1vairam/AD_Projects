...
# knitFile2Synapse(file = "./Takeda_DiffExpAnal.Rmd", owner = 'syn4059381', name = "Differential Expression Analysis",overwrite=F)
...
executed = 'https://github.com/th1vairam/Takeda_contract/blob/master/Rmd/Takeda_DiffExpAnal.Rmd';
```
...
differential.expression <- function(EXPR_ID, COV_ID){
  ...
  # Fit linear model
  FIT <- lmFit(EXPR,design = DESIGN$design, correlation = correlation$consensus.correlation, block = GENE.NAMES)
...
# Get fold change and pvalues
tmp <- DIFF.EXP$DEXP$logFC
colnames(tmp) <- paste('frontal.pole',colnames(tmp),sep='.')
FC <- join(FC, cbind(DIFF.EXP$ANNOTATIONS,tmp), by = 'ID', match = 'all')
...
# Get fold change and pvalues
tmp <- DIFF.EXP$DEXP$logFC
colnames(tmp) <- paste('hippocampal',colnames(tmp),sep='.')
FC <- join(FC, cbind(DIFF.EXP$ANNOTATIONS,tmp), by = 'ID', match = 'all')
